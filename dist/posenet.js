...
//#endregion
//#region src/posenet.ts
var poseNetModelDefaults = Object.freeze({
	architecture: "MobileNetV1",
	multiplier: .75,
	outputStride: 16,
	inputResolution: 257
});
var poseNetBaseUrl = "https://storage.googleapis.com/tfjs-models/savedmodel/posenet/mobilenet/float/075/";
var poseNetPackageAssetDirectory = "dist/posenet/mobilenet-v1-075-stride16";
var expectedFiles = Object.freeze([
	Object.freeze({
		...
		packageSpecifier: `${package_default.name}/posenet-assets/model-stride16.json`,
		maxBytes: 65536
	}),
	Object.freeze({
		...
		packageSpecifier: `${package_default.name}/posenet-assets/group1-shard1of2.bin`,
		maxBytes: 4194304
	}),
	Object.freeze({
		...
		packageSpecifier: `${package_default.name}/posenet-assets/group1-shard2of2.bin`,
		maxBytes: 1048576
	})
]);
var poseNetBundleManifest = Object.freeze({
	formatVersion: 1,
...
var PoseNetBundleError = class extends Error {
	code;
...
function abortError() {
	const error = /* @__PURE__ */ new Error("TM PoseNet model loading was cancelled.");
...
function requireSubtleCrypto(value) {
	if (!isRecord(value) || typeof value.digest !== "function") return fail("TM-POSENET-ASSET-001", "Web Crypto subtle.digest is required.");
	return value;
}
...
function encodeBase64(bytes) {
	if (typeof btoa !== "function") return fail("TM-POSENET-ASSET-001", "Base64 encoding requires btoa.");
	const chunks = [];
...
function hasValidBase64Alphabet(value) {
	...
	for (let index = 0; index < dataLength; index += 1) if (!isBase64Character(value.charCodeAt(index))) return false;
	return dataLength > 0;
}
function decodeBase64(value, label, maxBytes) {
	const maximumLength = Math.ceil(maxBytes / 3) * 4;
	if (typeof value !== "string" || value.length === 0 || value.length > maximumLength || value.length % 4 !== 0 || !hasValidBase64Alphabet(value)) return fail("TM-POSENET-ASSET-001", `${label} must be bounded padded Base64.`);
	if (typeof atob !== "function") return fail("TM-POSENET-ASSET-001", "Base64 decoding requires atob.");
	let decoded;
	try {
		decoded = atob(value);
	} catch (error) {
		return fail("TM-POSENET-ASSET-001", `${label} is invalid Base64: ${error instanceof Error ? error.message : String(error)}`);
	}
	if (decoded.length > maxBytes) return fail("TM-POSENET-ASSET-004", `${label} exceeds its decoded byte limit.`);
	const bytes = new Uint8Array(decoded.length);
	for (let index = 0; index < decoded.length; index += 1) bytes[index] = decoded.charCodeAt(index);
	return bytes;
}
function canonicalProjectBundle(descriptor) {
	if (!isRecord(descriptor) || descriptor.formatVersion !== 1 || descriptor.encoding !== "base64" || !Array.isArray(descriptor.files) || descriptor.files.length !== expectedFiles.length) return fail("TM-POSENET-ASSET-001", "PoseNet project bundle format is invalid.");
	const files = expectedFiles.map((expected) => {
		const candidate = descriptor.files.find((file) => isRecord(file) && file.path === expected.path);
		if (!isRecord(candidate) || candidate.mediaType !== expected.mediaType || candidate.sha256 !== expected.sha256 || !Number.isSafeInteger(candidate.size) || Number(candidate.size) !== expected.size || typeof candidate.data !== "string") return fail("TM-POSENET-ASSET-001", `PoseNet project file metadata is invalid: ${expected.path}.`);
...
function decodeProjectBundle(descriptor, signal) {
	...
	return Object.freeze(canonical.files.map((candidate, index) => {
		throwIfAborted(signal);
		const expected = expectedFiles[index];
		const bytes = decodeBase64(candidate.data, `PoseNet project file ${expected.path}`, expected.size);
		if (bytes.byteLength !== candidate.size) return fail("TM-POSENET-ASSET-004", `PoseNet project file size does not match: ${expected.path}.`);
		return Object.freeze({
			path: expected.path,
			mediaType: expected.mediaType,
			bytes
...
async function verifyPoseNetBundle(files, { subtleCrypto = globalThis.crypto?.subtle, signal } = {}) {
	throwIfAborted(signal);
	const digestRuntime = requireSubtleCrypto(subtleCrypto);
	if (!Array.isArray(files) || files.length !== expectedFiles.length) return fail("TM-POSENET-ASSET-004", "PoseNet bundle must contain exactly three files.");
	let totalBytes = 0;
	const candidates = expectedFiles.map((expected) => {
		const candidate = files.find((file) => isRecord(file) && file.path === expected.path);
		if (!candidate) return fail("TM-POSENET-ASSET-002", `PoseNet file is missing: ${expected.path}.`);
		const bytes = requireBytes(candidate.bytes, `PoseNet file ${expected.path}`);
		if (bytes.byteLength !== expected.size) return fail("TM-POSENET-ASSET-004", `PoseNet file size is invalid: ${expected.path}.`);
	...
	if (totalBytes > poseNetBundleManifest.limits.maxTotalBytes) return fail("TM-POSENET-ASSET-004", "PoseNet bundle exceeds its total byte limit.");
	const digests = await Promise.all(candidates.map(async ({ bytes }) => {
		throwIfAborted(signal);
		return sha256(bytes, digestRuntime);
	}));
	throwIfAborted(signal);
	const verifiedFiles = candidates.map(({ expected, bytes }, index) => {
		if (digests[index] !== expected.sha256) return fail("TM-POSENET-ASSET-003", `PoseNet file integrity mismatch: ${expected.path}.`);
		return Object.freeze({
			path: expected.path,
			url: expected.url,
			mediaType: expected.mediaType,
	...
	if (!jsonFile) return fail("TM-POSENET-ASSET-002", "PoseNet model JSON is missing.");
	let modelJson;
	try {
		modelJson = JSON.parse(new TextDecoder().decode(jsonFile.bytes));
	} catch (error) {
		return fail("TM-POSENET-ASSET-001", `PoseNet model JSON is invalid: ${error instanceof Error ? error.message : String(error)}`);
	}
	const manifestPaths = (isRecord(modelJson) && Array.isArray(modelJson.weightsManifest) ? modelJson.weightsManifest : []).flatMap((entry) => isRecord(entry) && Array.isArray(entry.paths) ? entry.paths : []);
	const expectedShardPaths = expectedFiles.filter(({ path }) => path.endsWith(".bin")).map(({ path }) => path);
	if (manifestPaths.length !== expectedShardPaths.length || expectedShardPaths.some((path) => !manifestPaths.includes(path))) return fail("TM-POSENET-ASSET-001", "PoseNet model JSON references unexpected weight shards.");
	return Object.freeze({
...
async function loadPoseNetBundle(loadFile, options = {}) {
	...
	return verifyPoseNetBundle(await Promise.all(expectedFiles.map(async (file) => {
...
async function createPoseNetProjectBundle(files, options = {}) {
	return createProjectBundleFromVerified(await verifyPoseNetBundle(files, options));
}
function createProjectBundleFromVerified(verified) {
	return Object.freeze({
		formatVersion: 1,
		encoding: "base64",
		files: Object.freeze(verified.files.map((file, index) => Object.freeze({
			path: file.path,
			mediaType: file.mediaType,
...
function validateRuntime(value) {
	if (!isRecord(value) || typeof value.Webcam !== "function" || typeof value.loadFromFiles !== "function") return fail("TM-POSENET-RUNTIME-001", "TM runtime must provide Webcam and loadFromFiles.");
	return value;
}
...
async function disposeRuntimeModel(value) {
	...
	const errors = (await Promise.allSettled(resources.map((resource) => Promise.resolve().then(() => resource.dispose())))).flatMap((result) => result.status === "rejected" ? [result.reason] : []);
	if (errors.length > 0) throw new AggregateError(errors, "TM could not dispose a cancelled runtime model.");
}
function createBundledTMRuntime(options) {
	if (!isRecord(options)) throw new TypeError("PoseNet runtime options are required.");
	if (options.parallelModelInitialization !== void 0 && typeof options.parallelModelInitialization !== "boolean") throw new TypeError("parallelModelInitialization must be a boolean.");
	...
	if (typeof ResponseConstructor !== "function") return fail("TM-POSENET-RUNTIME-001", "Response constructor is required.");
	if ([
		options.files,
		options.loadFiles,
		options.projectBundle
	].filter((value) => value !== void 0).length !== 1) throw new TypeError("Provide exactly one of files, loadFiles, or projectBundle.");
	let verification;
	const verifySupply = () => {
		verification ??= options.projectBundle ? loadPoseNetProjectBundle(options.projectBundle, { subtleCrypto: options.subtleCrypto ?? globalObject.crypto?.subtle }) : Promise.resolve(options.loadFiles ? options.loadFiles() : options.files).then((files) => verifyPoseNetBundle(files, { subtleCrypto: options.subtleCrypto ?? globalObject.crypto?.subtle }));
		return verification;
	};
	...
	async function loadFromFiles(model, weights, metadata, loadOptions = {}) {
		const task = async () => {
			...
			if (typeof previousFetch !== "function") return fail("TM-POSENET-RUNTIME-001", "Browser fetch is required.");
			...
			const localFetch = async (input) => {
				...
				if (!file) return fail("TM-POSENET-FETCH-001", `Unexpected PoseNet request: ${url ?? "(invalid)"}.`);
			...
			try {
				...
				return fail("TM-POSENET-FETCH-001", `PoseNet fetch interception is unavailable: ${error instanceof Error ? error.message : String(error)}`);
			}
			try {
				let runtimeLoad;
				try {
					runtimeLoad = Promise.resolve(runtime.loadFromFiles(model, weights, metadata, {
						...loadOptions,
				...
				let disposalError;
				if (runtimeResult.status === "fulfilled" && (loadOptions.signal?.aborted || supplyResult.status === "rejected")) try {
				...
				if (loadOptions.signal?.aborted) {
					...
					if (runtimeError || disposalError) throw new AggregateError([
						cancellation,
						runtimeError,
						disposalError
					].filter((error) => error !== void 0), "TM cancellation encountered a runtime or disposal failure.");
					throw cancellation;
				}
				if (supplyResult.status === "rejected") {
					if (disposalError) throw new AggregateError([supplyResult.reason, disposalError], "PoseNet verification failed and model cleanup was incomplete.");
					throw supplyResult.reason;
				}
				if (runtimeResult.status === "rejected") throw runtimeResult.reason;
				return runtimeResult.value;
			} finally {
				if (globalObject.fetch === localFetch) try {
...
//#endregion
export { PoseNetBundleError, createBundledTMRuntime, createPoseNetProjectBundle, createPoseNetProjectBundleFromLoader, loadPoseNetBundle, loadPoseNetProjectBundle, poseNetBundleManifest, poseNetModelDefaults, validatePoseNetProjectBundle, verifyPoseNetBundle };
